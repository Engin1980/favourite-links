...
public class KeycloakService {
  ...
  public String createUser(String email, String password, boolean isAdmin) {
    ...
    assignGroupToUser(keycloakUserId, role, adminToken);

    return keycloakUserId;
  }

  private String extractGroupId(String json, String groupName) {
    // Jednoduchý parser pro nalezení ID skupiny podle názvu
    String[] groups = json.split("\\{");
    for (String group : groups) {
      if (group.contains("\"name\":\"" + groupName + "\"")) {
        String[] parts = group.split("\"id\":\"");
        if (parts.length > 1) {
  ...
  private void assignGroupToUser(String userId, String groupName, String adminToken) {
    // Nejprve získáme ID skupiny
    URI groupUrl = createUri(String.format("%s/admin/realms/%s/groups", serverUrl, realm));
    ...
    HttpResponse<String> groupResponse = getHttpResponse(groupRequest,
        e -> new ServiceException(this, String.format("Failed to get groups"), e));

    if (groupResponse.statusCode() != 200) {
      throw new ServiceException(this, "Failed to fetch groups");
    }

    // Najdeme ID skupiny podle názvu (jednoduchý parsing JSON)
    String groupId = extractGroupId(groupResponse.body(), groupName);
    if (groupId == null) {
      throw new ServiceException(this, String.format("Group %s not found", groupName));
    }

    // Přiřadíme skupinu uživateli
    URI assignUrl = createUri(String.format("%s/admin/realms/%s/users/%s/groups/%s",
    ...
    HttpResponse<String> assignResponse = getHttpResponse(assignRequest,
        e -> new ServiceException(this, String.format("Failed to assign group %s to user %s", groupName, userId), e));

    if (assignResponse.statusCode() != 204) {
      throw new ServiceException(this,
          String.format("Failed to assign group with status %d: %s",
